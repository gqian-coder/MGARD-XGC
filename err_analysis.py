...
#with ad2.open('/Users/qg7/Documents/Projs/XGC Fusion/data files/low_res/d3d_coarse_v2/decompressed_untwisted_4d.bp', 'r') as f:
    f_twisted_rct = f.read('i_f')
f_twisted_rct = np.moveaxis(np.reshape(f_twist_rct, [8, 39,nnodes,39]), 1,2)
...
for i in range(steps):
    for j in range(f_twisted_sf[i].shape[1]):
        f_twisted_rct_nodes.append(np.sum(f_twisted_sf_rct[i][:,j,:,:]))
        L_inf.append(np.max(abs(f_twisted_sf_rct[i][:,j,:,:] - f_twisted_sf_ori[i][:,j,:,:])))
        L_inf_perc.append(L_inf[-1]/np.max(f_twisted_sf_ori[i][:,j,:,:])) 
...
plt.figure()
plt.plot(f_twisted_ori_nodes, label = 'twisted data')
plt.plot(f_twisted_sf_rct_nodes, label = 'reconstruction by flux surface')
plt.legend()
plt.savefig('point_wise_err.eps')
plt.close()
plt.figure()
plt.plot(L_inf, label = 'by flux surface reconstruction')
plt.plot(err_ori, label = 'original reconstruction')
plt.plot(err_twisted, label = 'twisted data reconstruction')
plt.legend()
...
plt.plot(err_ori_perc, label = 'original reconstruction')
plt.plot(err_twisted_perc, label = 'twisted data reconstruction')
plt.legend()
