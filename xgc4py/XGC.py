...
class XGC:
    ...
    def f0_diag(self, f0_inode1, ndata, isp, f0_f, progress=False):
        ...
        u_para_ = u_para_.astype(np.float128)/s_den_[:,np.newaxis,np.newaxis]
        T_perp_ = T_perp_.astype(np.float128)/s_den_[:,np.newaxis,np.newaxis]/sml_e_charge

        # 2) T_para moment
        upar_ = np.sum(u_para_, axis=(1,2), dtype=np.float128)/vth
        en_   = 0.5 * (vp[np.newaxis,:].astype(np.float128) - upar_[:,np.newaxis])**2

        T_para_ = f0_f.astype(np.float128) * vol_ * en_[:,np.newaxis,:] * vth2[:,np.newaxis,np.newaxis] * ptl_mass[isp]
        T_para_ = 2.0*T_para_/s_den_[:,np.newaxis,np.newaxis]/sml_e_charge

        n0_ = s_den_
        T0_ = (2.0*np.sum(T_perp_, axis=(1,2), dtype=np.float128)+np.sum(T_para_, axis=(1,2), dtype=np.float128))/3.0
